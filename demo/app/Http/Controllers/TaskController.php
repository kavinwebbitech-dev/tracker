...
class TaskController extends Controller
{
    ...
    public function TaskCreate()
    {
        ...
        return view('admin.task.add', compact('sub_admin', 'sub_admin1', 'projects', 'staff_details'));
    }
    ...
    public function AdminStaffGet(Request $request)
    {
        ...
        $sub_admin1 = User::where('user_type', $user_type)->get();
        // dd($sub_admin1);
        $html = '';
        ...
        if ($task_details->save()) {
            ...
            try{
                Mail::send('mail.task_details', $data, function($message)use($data) {
                    $message->to($data['staff_details']['email'], $data['staff_details']['name'])
                        ->from("noreplywebbitech@gmail.com", 'no-reply')
                    ->subject(date('d-m-Y', strtotime($data['task_details']['start_date'])) .' - '. date('d-m-Y', strtotime($data['task_details']['end_data'])). ' Recurring Task has been '.$data['status1'].' by Super Admin');
                });
            }
            catch(\Exception $e){
                // dd($e);
        ...
        if ($task_comments->save()) {
            ...
            try{
                Mail::send('mail.custom_task_status', $data, function($message)use($data) {
                    $message->to($data['staff_details']['email'], $data['staff_details']['name'])
                        ->from("noreplywebbitech@gmail.com", 'no-reply')
                    ->subject($data['task_details']['name']. ' task has been '.$data['status1'].' By Super Admin');
                });
            }
            catch(\Exception $e){
                // dd($e);

...
@section('content')
	...
	<?php
        $total_task   = \App\Models\Task::where('staff_id', Auth::user()->id)->get();
        $pending        = \App\Models\Task::where('staff_id', Auth::user()->id)->where('status', 'pending')->get();
        $progress       = \App\Models\Task::where('staff_id', Auth::user()->id)->where('status', 'progress')->get();
        $completed      = \App\Models\Task::where('staff_id', Auth::user()->id)->where('status', 'completed')->get();
        $rejected       = \App\Models\Task::where('staff_id', Auth::user()->id)->where('status', 'rejected')->get();
    ?>
				...
				<div class="box">
	                ...
	                <div class="box-body">
	                	<div class="box no-shadow">		
							<form class="form-horizontal form-element col-12" action="{{ route('staff.profile.upload', $user->id) }}" method="post" enctype="multipart/form-data">
								@csrf
								  ...
								  <div class="form-group row">
									<label for="inputPhone" class="col-sm-2 form-label">Joining Date</label>

									<div class="col-sm-10">
									  <input type="date" class="form-control" id="inputPhone" name="join_date" placeholder="" value="{{ $user->join_date }}">
									</div>
								  </div>
								  <div class="form-group row">
					  ...
					  <div class="row">
						...
						<div class="col-sm-4 be-1 bs-1">
						  <div class="description-block">
							...
							<span class="description-text">Pending Task</span>
						  </div>
						  <!-- /.description-block -->
						</div>
						<!-- /.col -->
						<div class="col-sm-4">
						  <div class="description-block">
							<h5 class="description-header">{{ count($completed) }}</h5>
							<span class="description-text">Complete Task</span>
						  </div>
						  <!-- /.description-block -->
						</div>
						<!-- /.col -->

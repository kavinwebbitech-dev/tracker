...
<!-- Content Wrapper. Contains page content -->
            ...
            <div class="col-12">
                ...
                <div class="box-body">
                    ...
                    <div class="row" style="margin-bottom: 20px;">
                        ...
                        <div class="col-sm-8">
                            <a class="btn btn-primary" href="{{ route('admin.amc.create') }}" style="float: right;">Add New</a>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-10"></div>
                        <div class="col-md-2">
                            <input type="text" name="text_value_search" id="text_value_search" class="form-control" onkeyup="text_value_search()" style="position: absolute;width: 200px;z-index: 99;">
                        </div>
                    </div>
                    <style type="text/css">
                        .add_button
                    ...
                    <div class="table-responsive">
                      <table id="example" class="table table-bordered table-hover display nowrap margin-top-10 w-p100">
                        ...
                        <tbody>
                            ...
                            <?php
                                $gsuite_end_date = $value->fld_amc_end_date;
                                $today=date('Y-m-d');
                            ...
                            <tr @if($gsuite_days == 0) style="background: #f6e2f7;" @endif>
                                ...
                                <td>
                                    <?php
                                        $customer_details = \App\Models\Customers::where('id', $value->fld_cust_id)->first();
                                        $total_amount += $value->fld_amc_total_amount;
                                    ?>
                                    {{ $customer_details->fld_company_name }}</td>
                                <td>{{ date('d-m-Y', strtotime($value->fld_amc_reg_date)) }}</td>
                                <td>{{ date('d-m-Y', strtotime($value->fld_amc_end_date)) }}</td>
                                <td>{{ $value->fld_amc_dh_tenure }}</td>
                                <td>
                                    {{ $gsuite_days_show }}
                                </td>
                                <td>
                                    {{ $value->fld_amc_total_amount }}
                                </td>
                                <td>
                                    <p style="display: flex;">
                                        <a href="{{ route('admin.amc.edit1', $value->id) }}" type="Edit" class="btn btn-success" style="padding: 4px 12px;margin: 0px 4px;"><i class="ion ion-edit text-white"></i></a>
                                        <a href="{{ route('admin.amc.not.interest', $value->id) }}" data-confirm="Are you Sure Move Not Interest?" title="Not interest" class="delete btn btn-warning" style="padding: 4px 12px;margin: 0px 4px;"><i class="ti-unlink text-white"></i></a>
                                        <a href="{{ route('admin.amc.invoice', $value->id) }}" title="Invoice" class="btn btn-dark" style="padding: 4px 12px;margin: 0px 4px;" target="_blank"><i class="ti-files text-white"></i></a>
                                        ...
                                        <a href="{{ route('admin.amc.delete', $value->id) }}" title="Delete" class="btn btn-danger" style="padding: 4px 12px;margin: 0px 4px;"><i class="ti-trash text-white"></i></a>
                                    </p>
                                </td>
                            </tr>
                            @endforeach
  ...
  <script type="text/javascript">
    ...
    function CustomerDetails(id) {
        ...
        if (project_id) {
            $.ajax({
                url: "{{ route('admin.amc.user.details') }}",
                type: "POST",
                data: {
                    id: project_id,
                    _token: '{{csrf_token()}}'
            ...
            if($(this).is(':checked')) {      
                ...
                $.fn.dataTable.ext.search.push(
                    function( settings, data, dataIndex ) {
                        return (parseFloat(data[5])<30 || parseFloat(data[6])<30)
                            ? true
                            : false
                    }     
                );                                     

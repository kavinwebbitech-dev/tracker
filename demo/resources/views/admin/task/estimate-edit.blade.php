			...
			<div class="row">			  
				...
				<div class="col-lg-12 col-12">
                    ...
                    <style type="text/css">
					  ...
					  <div class="box">
						<!-- /.box-header -->
						<form class="form" action="{{ route('admin.task.request.edit.store', $task_details->id) }}" method="post" enctype="multipart/form-data">
							@csrf
							<div class="box-body">
											...
											<div class="input-group in-bord mb-3">
												<select class="form-control select2 @error('client_id') is-invalid @enderror" name="client_id" id="client_id">
													...
													@foreach($client_details as $key => $value)
														<option value="{{ $value->id }}" @if($task_details->user_id == $value->id) selected @endif> {{ $value->name }} ({{ $value->phone }}) @if($value->client_details->fld_company_name) - {{ $value->client_details->fld_company_name }} @endif</option>
													@endforeach
												</select>
						                ...
						                <div class="col-12 text-center" id="dynamicTable">
						                    ...
						                    <div class="row mb-3 @if($key == 0) @else dynamicrow @endif">
						                        <div class="col-md-10">
									                ...
									                <textarea name="addmore[{{ $key }}][points]" class="form-control" rows="2" style="width: 100%;">
									                	{{ $value->name }}
									                </textarea>
									            </div>
									            @if($key == 0)

                    ...
                    <div class="table-responsive">
                      <table class="table table-bordered table-hover display nowrap margin-top-10 w-p100">
                        ...
                        <tbody>
                            ...
                            ?>
                            <tr @if($value->status == "Approved") style="background: #90EE90;" @elseif($value->status == "Closed") style="background: #f94381;" @endif>
                                <td>{{ $key + 1 }}</td>
                                ...
                                <td>
                                    @if($value->client_task_details[0]->amount_type == "fixed_amount")
                                        @if($total_amount) ₹ {{ number_format($total_amount, 2) }}@endif
                                    @else
                                        @if($total_amount) $ {{ number_format($total_amount, 2) }}@endif
                                    @endif
                                </td>
                                <td>
                                    @if($value->client_task_details[0]->amount_type == "fixed_amount")
                                        @if($total_amount1) ₹ {{ number_format($total_amount1, 2) }}@endif
                                    @else
                                        @if($total_amount1) $ {{ number_format($total_amount1, 2) }}@endif
                                    @endif
                                </td>
                                ...
                                <td>
                                    <p style="display: flex;margin-bottom: 0px;">
                                        <a href="{{ route('admin.task.request.edit', $value->id) }}" class="btn btn-primary" style="padding: 4px 12px;margin: 0px 4px;"><i class="ion ion-edit text-white"></i></a>
                                        <a href="{{ route('task.request.details', $value->id) }}" class="btn btn-success" style="padding: 4px 12px;margin: 0px 4px;"><i class="ti-eye text-white"></i></a>
                                        <a href="{{ route('admin.projects.task.estimate.delete', $value->id) }}" class="delete btn btn-danger" data-confirm="Are you Sure Delete this Task Estimate?" style="padding: 4px 12px;margin: 0px 4px;"><i class="ti-trash text-white"></i></a>

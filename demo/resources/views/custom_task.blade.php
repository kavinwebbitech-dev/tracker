<table>
    ...
    @foreach($invoices as $key => $invoice)
        <tr>
            <td>{{ $key + 1 }}</td>
            <td>{{ $invoice->task_details->task_no }}</td>
            <td>{{ $invoice->task_details->assign_details->name ?? '' }}</td>
            <td>{{ $invoice->staff_details->name ?? '' }}</td>
            <td>{{ $invoice->task_details->name }}</td>
            <td style="width: 400px;" word-wrap>{!! $invoice->description !!}</td>

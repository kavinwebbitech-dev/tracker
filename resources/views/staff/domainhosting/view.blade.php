...
<!-- Content Wrapper. Contains page content -->
            ...
            <div class="col-12">
                ...
                <div class="box-body">
                    ...
                    <div class="row" style="margin-bottom: 20px;">
                        ...
                        @if(in_array('22', $role_section))
                        <div class="col-sm-8">
                            <a class="btn btn-primary" href="{{ route('staff.domain.hosting.create') }}" style="float: right;">Add New</a>
                        </div>
                        @endif
                    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <p style="display: flex;">
                                        ...
                                        <a href="{{ route('staff.domain.hosting.edit1', $value->id) }}" type="Edit" class="btn btn-success" style="padding: 4px 12px;margin: 0px 4px;"><i class="ion ion-edit text-white"></i></a>
                                        @endif
                                        @if(in_array('26', $role_section))
                                        <a href="{{ route('staff.domain.hosting.not.interest', $value->id) }}" data-confirm="Are you Sure Move Not Interest?" title="Not interest" class="delete btn btn-warning" style="padding: 4px 12px;margin: 0px 4px;"><i class="ti-unlink text-white"></i></a>
                                        @endif
                                        @if(in_array('24', $role_section))
                                       <a href="javascript:void(0);"onclick="deleteProject('{{ route('staff.domain.hosting.delete', $value->id) }}')" title="Delete" class="btn btn-danger" style="padding: 4px 12px;margin: 0px 4px;"><i class="ti-trash text-white"></i></a>
                                        @endif
                                        @if(in_array('25', $role_section))
                                        <a href="{{ route('staff.domain.hosting.invoice', $value->id) }}" title="Invoice" class="btn btn-dark" style="padding: 4px 12px;margin: 0px 4px;"><i class="ti-files text-white"></i></a>
                                        @endif
  ...
  <script type="text/javascript">
    ...
    function CustomerDetails(id) {
        ...
        if (project_id) {
            $.ajax({
                url: "{{ route('staff.domain.hosting.user.details') }}",
                type: "POST",
                data: {
                    id: project_id,
                    _token: '{{csrf_token()}}'

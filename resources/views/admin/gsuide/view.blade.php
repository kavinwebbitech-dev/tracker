...
@section('content')
    ...
    <div class="content-wrapper">
        <div class="container-full">
            ...
            // the controller via start_date / month), grouped by fld_gsuite_end_date
            // so it matches the "Expiry Date" column shown in the table.
            $year_month_wise_total = [];
            if ($domainhosting) {
                foreach ($domainhosting as $value) {
                    $year_key = date('Y', strtotime($value->fld_gsuite_end_date));
                    $month_key = date('m', strtotime($value->fld_gsuite_end_date));
                    $month_label = date('F', strtotime($value->fld_gsuite_end_date));
                        ...
                        <div class="box">
                            ...
                            <div class="box-body">
                                ...
                                <div class="row" style="margin-bottom: 20px;">
                                    ...
                                    <div class="col-sm-4">
                                        <a class="btn btn-primary" href="{{ route('admin.gsuide.create') }}"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <input type="text" name="text_value_search" id="text_value_search"
                                            ...
                                            style="position: absolute;width: 200px;z-index: 9999;">
                                    </div>
                                </div>
                                <style type="text/css">
                                    .add_button {
                                            ...
                                            @if ($domainhosting)
                                                @foreach ($domainhosting as $key => $value)
                                                    ...
                                                    <tr
                                                        ...
                                                        <td>{{ $key + 1 }}</td>
                                                        <td class="gs-st">{{ $value->fld_domain_name }}</td>
                                                        <td class="gs-st"><i class="bi bi-calendar-fill text-success me-1"></i>{{ date('d-m-Y', strtotime($value->fld_gsuite_end_date)) }}
                                                        </td>
                                                        <td class="gs-st"><i class="bi bi-calendar-range text-success me-1"></i>
                                                            {{ $gsuite_days_show }}
                                                        </td>
                                                        <td class="gs-st"><i class="bi bi-currency-rupee text-success me-1"></i>
                                                            {{ $value->fld_total_amount }}
                                                        </td>
                                                        <td>
                                                            <p style="display: flex;">
                                                                <a href="{{ route('admin.gsuide.edit1', $value->id) }}"
                                                                    type="Edit" class="btn btn-success"
                                                                    style="padding: 4px 12px;margin: 0px 4px;"><i
                                                                        class="ion ion-edit text-white"></i></a>
                                                                <a href="{{ route('admin.gsuide.not.interest', $value->id) }}"
                                                                    data-confirm="Are you Sure Move Not Interest?"
                                                                    title="Not interest" class="delete btn btn-warning"
                                                                    style="padding: 4px 12px;margin: 0px 4px;"><i
                                                                        class="ti-unlink text-white"></i></a>
                                                                <a href="{{ route('admin.gsuide.invoice', $value->id) }}"
                                                                    title="Invoice" class="btn btn-dark"
                                                                    style="padding: 4px 12px;margin: 0px 4px;"
                                                                    target="_blank"><i
                                                                        class="ti-files text-white"></i></a>
                                                                <a onclick="CustomerDetails('{{ $value->fld_cust_id }}')"
                                                                    title="Customer Details" class="btn btn-primary"
                                                                    style="padding: 4px 12px;margin: 0px 4px;"><i
                                                                        class="ti-user text-white"></i></a>
                                                                <a href="javascript:void(0);" onclick="deleteProject('{{ route('admin.gsuide.delete', $value->id) }}')"

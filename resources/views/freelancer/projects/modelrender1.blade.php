        ...
        <form action="{{ route('freelancer.task.request.comment.update') }}" method="post">
            ...
            <div class="row">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            <label class="form-label">Working Hour(s)</label>
                            <div class="input-group in-bord mb-3">
                                <input type="number" class="form-control @error('hour') is-invalid @enderror" name="hour" placeholder="Hour" value="{{ old('hour') }}" required>
                                {{-- <input type="time" 
                                name="total_time" 
                                class="form-control @error('total_time') is-invalid @enderror" 
                                value="{{ old('total_time', $update_task->total_time ?? '') }}" 
                                step="900"
                                required> --}}
                            </div>
                            @error('hour')

...
@section('content')
	...
	<?php
        $total_task   = \App\Models\user::where('admin_id', Auth::user()->id)->get();
        $pending        = \App\Models\Task::where('admin_id', Auth::user()->id)->where('status', 'pending')->get();
        $progress       = \App\Models\Task::where('staff_id', Auth::user()->id)->where('status', 'progress')->get();
        $completed      = \App\Models\Task::where('admin_id', Auth::user()->id)->where('status', 'completed')->get();
				...
				<div class="box">
	                ...
	                <div class="box-body">
	                	<div class="box no-shadow">		
							<form class="form-horizontal form-element col-12" action="{{ route('sub.admin.profile.upload', $user->id) }}" method="post" enctype="multipart/form-data">
								@csrf

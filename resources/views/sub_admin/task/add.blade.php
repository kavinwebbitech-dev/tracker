...
@section('content')
                            ...
                            <form class="form" action="{{ route('sub.task.store') }}" method="post">
                                ...
                                <div class="box-body">
                                    <div class="row">
                                        ...
                                        <!-- ONE TIME TASK: Select Staff (multiselect, role-aware) -->
                                                    ...
                                                    <select
                                                        ...
                                                        @if ($sub_admin)
                                                            @foreach ($sub_admin as $key => $staff_details)
                                                                <option value="{{ $staff_details->id }}"
                                                                    data-role="{{ $staff_details->role }}">
                                                                    {{ $staff_details->name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
                                        ...
                                        <!-- RECURRING: Select Staff (single dropdown) -->
                                    ...
                                    <div class="row" id="customer_choice_options" style="display:none;"></div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <textarea id="editor1" name="editor1" rows="10" cols="80" required></textarea>
                                            </div>
                                        </div>
                                    </div>
    ...
    <script type="text/javascript">
        ...
        // ─── Role → hour field mapping (same as admin) ─────────────────────────
        const roleHourMap = {
        ...
        function cusom_date() {
            ...
            if (date_check == "custom") {
                ...
                // FIX: rebuild/refresh the staff select now that it's visible,
                // otherwise the multiselect plugin renders it empty ("Nothing selected")
                // because it was initialized while #single_staff had display:none.
                if ($.fn.select2) {
                    $('#multiple_staff').select2('destroy');
                    $('#multiple_staff').select2({
                        width: '100%'
                    });
                }
                if ($('#multiple_staff').data('multiselect')) {
                    $('#multiple_staff').multiselect('rebuild');
                }

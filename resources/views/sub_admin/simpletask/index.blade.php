...
@section('content')
                        ...
                        <div class="box">
                            ...
                            <div class="box-body">
                                ...
                                <div class="st-header d-flex justify-content-between align-items-end flex-wrap gap-3 mb-3">
                                    ...
                                    <form action="{{ route('sub_admin.simpletask.view') }}" method="GET"
                                    ...
                                    <table id="simpleTaskTable" class="table table-bordered table-striped nowrap-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Title</th>
                                                <th>Description</th>
                                                @if (in_array('553', $role_section) || in_array('554', $role_section) || in_array('555', $role_section))
                                                    <th>Action</th>
                                                @endif
                                            </tr>
                                        </thead>
                                        <tbody id="st-table-body">
                                            @forelse($simple_tasks as $key => $task)
                                                <tr id="st-row-{{ $task->id }}">
                                                    ...
                                                    @if (in_array('553', $role_section) || in_array('554', $role_section) || in_array('555', $role_section))
                                                        <td>
                                                            @if (in_array('555', $role_section))
                                                                <p style="display: flex; margin: 0;">
                                                                    <a href="{{ route('sub.task.create', [
                                                            ...
                                                            @endif
                                                            @if (in_array('554', $role_section))
                                                                <a href="javascript:void(0);"
                                                                    onclick="deleteProject('{{ route('sub_admin.simpletask.delete', $task->id) }}')"
    ...
    <div class="modal fade" id="taskModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="taskForm" method="POST" action="{{ route('sub_admin.simpletask.store') }}">
                    @csrf
    ...
    <script type="text/javascript">
        ...
        function openAddModal() {
            $('#taskForm')[0].reset();
            $('#taskForm').attr('action', "{{ route('sub_admin.simpletask.store') }}");
            $('#task_method').val('');
        ...
        function openEditModal(link) {
            var id = $(link).data('id');
            var updateUrl = "{{ route('sub_admin.simpletask.update', ':id') }}".replace(':id', id);

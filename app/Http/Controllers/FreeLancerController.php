...
class FreeLancerController extends Controller
{
    ...
    {
        $sub_admin = User::where('user_type', 'freelancer')->where('status', 'active')->get();
    ...
    {
        $client_details = User::where('user_type', 'freelancer')->where('status', 'active')->get();
    ...
    {
        $sub_admin = User::where('user_type', 'freelancer')->where('status', 'active')->get();
        return view('sub_admin.freelancer.add', compact('sub_admin'));
    }

    public function subview()
    {
        $sub_admin = User::where('user_type', 'freelancer')->where('status', 'active')->get();
        return view('sub_admin.freelancer.view', compact('sub_admin'));
    }
    ...
    public function subedit($id)
    {
        $staff = User::where('id', $id)->where('status', 'active')->first();
        $sub_admin = User::where('user_type', 'freelancer')->where('status', 'active')->get();
        return view('sub_admin.freelancer.edit', compact('sub_admin', 'staff'));
    }
    ...
    public function subdelete($id)
    {
        if ($id) {
            $user  = User::find($id);
            if ($user->delete()) {
                return redirect()->route('sub_admin.freelancer.view')->with('error', 'Staff Deleted Successfully');
            }
            else{
                return redirect()->back()->with('error', 'Something Wrong');
            }
    ...
    public function substore(Request $request)
    {
        ...
        if ($user->save()) {
            ...
            return redirect()->route('sub_admin.freelancer.view')->with('success', 'Freelancer Created Successfully');
        }
        else{
            return redirect()->back()->with('error', 'Something Wrong');
        }
    ...
    public function subupdate(Request $request, $id)
    {
        ...
        if ($user->save()) {
            return redirect()->route('sub_admin.freelancer.view')->with('warning', 'Freelancer Edited Successfully');
        }
        else{
            return redirect()->back()->with('error', 'Something Wrong');
        }
    ...
    public function subTaskRequest()
    {
        $client_task  = ClientTask::where('user_type', 'freelancer')->get();
        $client_task1 = ClientTask::where('user_type', 'freelancer')->whereNot('status', 'Closed')->get();
        ...
        return view('sub_admin.freelancer.task-request', compact('client_task'));
    }

    public function subTaskRequestDetails($id)
    {
        ...
        return view('sub_admin.freelancer.requestview', compact('client_task'));
    }

    public function subTaskRequestUpdate(Request $request)
    {
        ...
        $view =  view('sub_admin.freelancer.modelrender1', compact('amount_details'))->render();
        return response()->json(['html'=>$view, 'background' => $background, 'request_view' => $request_view]);
    }

    public function subDeleteDetailsTask($id)
    {
    ...
    public function subTaskRequestCommentUpdate(Request $request)
    {
        // dd($request->all());
        ...
        if($request->status == "Approved")
        {
            ...
            $notification->task_id       = "";
            $notification->receiver_id   = 1;
            $notification->url           = route('sub_admin.freelancer.task.details', $client_task_details->client_task_id);
            $notification->receiver_type = "freelancer";
            $notification->message       = $message_cmd;
            $notification->status        = 0;
            $notification->save();
    ...
    public function subDeleteEstimateTask($id)
    {
        ClientTask::findOrFail($id)->delete();
        return redirect()->back()->with('error', 'Estimate Deleted Successfully');
    ...
    public function subEstimateCreate()
    {
        $client_details = User::where('user_type', 'freelancer')->where('status', 'active')->get();
        return view('sub_admin.freelancer.estimate-add', compact('client_details'));
    }

    public function subEstimateStore(Request $request)
    {
        // dd($request->all());
        ...
        if ($create_task->save()) {
            ...
            return redirect()->route('sub_admin.freelancer.request')->with('success', 'New Task Estimate Create SuccessFully');
    ...
    public function subEstimateExtraStore(Request $request)
    {
        // dd($request->all());
    ...
    public function subDeleteEstimateComment($id)
    {
    ...
    public function subTaskEstimateClose(Request $request)
    {
    ...
    public function subTaskRequestEdit($id)
    {
        $client_details = User::where('user_type', 'freelancer')->where('status', 'active')->get();
        $task_details = ClientTask::where('id', $id)->first();
        // dd($task_details);
        return view('sub_admin.freelancer.estimate-edit', compact('client_details', 'task_details'));
    }

    public function subEstimateEditStore(Request $request, $id)
    {
        ...
        if ($create_task->save()) {
            ...
            return redirect()->route('sub_admin.freelancer.request')->with('warning', 'Task Estimate Edited SuccessFully');
    ...
    public function subEstimateDescription(Request $request)
    {
        // dd($request->all());
        
        $update_task = ClientTask::where('id', $request->id)->first();
        
        $view =  view('sub_admin.freelancer.modelrender3', compact('update_task'))->render();
        return response()->json(['html'=>$view]);
        
    }

    public function subEstimatePay(Request $request){
        $update_task = ClientTask::where('id', $request->id)->first();
        
        $view =  view('sub_admin.freelancer.amount_pay', compact('update_task'))->render();
        return response()->json(['html'=>$view]);
    }

    public function subTaskRequestPaymentUpdate(Request $request){
        $client = ClientTask::find($request->client_task_id);
        if($client){
            $client->paid_amount = $client->paid_amount + $request->amount;
            $client->save();
            return redirect()->route('sub_admin.freelancer.request')->with('warning', 'Task Estimate Payment Updated SuccessFully');
        }
        return redirect()->route('sub_admin.freelancer.request')->with('danger', 'Task Estimate Not Found');
    }
